    ...
    if (autoSuggest) {
      ...
      return NextResponse.json({
        suggestedMusicians: suggested.map(m => ({
          ...
          name: m.name,
          phone: m.phone,
          instruments: m.profile?.instruments ? JSON.parse(m.profile.instruments) : [],
          vocals: m.profile?.vocals ? JSON.parse(m.profile.vocals) : [],
          penaltyPoints: m.penaltyPoints,
          totalEvents: m.profile?.totalEvents || 0,
        })),
