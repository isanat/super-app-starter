...
export async function POST(
  ...
  try {
    ...
    const singers = availableMusicians.filter(m => 
      m.role === "SINGER" || 
      (m.MusicianProfile?.vocals && JSON.parse(m.MusicianProfile.vocals).length > 0)
    )

    const instrumentalists = availableMusicians.filter(m => 
      m.role === "INSTRUMENTALIST" || 
      m.role === "MUSICIAN" ||
      (m.MusicianProfile?.instruments && JSON.parse(m.MusicianProfile.instruments).length > 0)
    )
    ...
    const suggestedScale = {
      singers: singers.slice(0, 4).map(s => ({
        ...
        email: s.email,
        phone: s.phone,
        vocals: s.MusicianProfile?.vocals ? JSON.parse(s.MusicianProfile.vocals) : [],
        penaltyPoints: s.penaltyPoints,
        reliability: (s.MusicianProfile?.totalEvents || 0) - (s.penaltyPoints || 0)
      })),
      instrumentalists: instrumentalists.slice(0, 6).map(i => ({
        ...
        email: i.email,
        phone: i.phone,
        instruments: i.MusicianProfile?.instruments ? JSON.parse(i.MusicianProfile.instruments) : [],
        penaltyPoints: i.penaltyPoints,
        reliability: (i.MusicianProfile?.totalEvents || 0) - (i.penaltyPoints || 0)
      })),
      period,
